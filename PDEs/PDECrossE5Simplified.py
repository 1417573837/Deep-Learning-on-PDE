...
@cacheout.memoize(maxsize=10000)
def f(x, y, p):
    with tf.GradientTape(watch_accessed_variables=False, persistent=True) as g:
        g.watch([x, y])
        with tf.GradientTape(watch_accessed_variables=False, persistent=True) as gg:
...
def true_sol_x_plotting(x, y):
    return tf.where(x > ksi,
                    tf.where(y > zeta, true_sol_x(x, y, 2), true_sol_x(x, y, 3)),
                    tf.where(y > zeta, true_sol_x(x, y, 1), true_sol_x(x, y, 4)))


def true_sol_y_plotting(x, y):
    return tf.where(x > ksi,
                    tf.where(y > zeta, true_sol_y(x, y, 2), true_sol_y(x, y, 3)),
                    tf.where(y > zeta, true_sol_y(x, y, 1), true_sol_y(x, y, 4)))